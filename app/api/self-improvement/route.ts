...
import { generateSelfImprovementSuggestions } from "@/app/lib/ai-task-planner";
...
export async function GET() {
  try {
    ...
    const suggestions = await generateSelfImprovementSuggestions({
      tasks: Array.isArray(tasks) ? tasks.slice(-25) : [],
      activity: Array.isArray(activity) ? activity.slice(0, 40) : [],
      memory,
