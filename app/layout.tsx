          ...
          <div className="flex min-h-screen overflow-x-hidden bg-neutral-950 text-white">
            <aside className="hidden w-64 shrink-0 border-r border-white/10 p-4 md:block">
              ...
              <nav className="space-y-2">
                {navItems.map((item) => (
                  <Link key={item.href} href={item.href} className="block rounded-lg px-3 py-2 text-sm text-white/70 transition hover:bg-white/10 hover:text-white">
                    {item.label}
                  </Link>
                ))}
              </nav>
              ...
              <div className="sticky top-0 z-20 border-b border-white/10 bg-neutral-950/95 backdrop-blur md:hidden">
                <div className="px-4 py-3">
                  ...
                  <nav className="flex flex-wrap gap-2">
                    {navItems.map((item) => (
                      <Link key={item.href} href={item.href} className="rounded-lg border border-white/10 px-3 py-2 text-sm text-white/70 transition hover:bg-white/10 hover:text-white">
                        {item.label}
                      </Link>
                    ))}
                  </nav>
